...
export function sessionMiddleware(db, config) {
  ...
  const LOOKUP = `SELECT s.id AS session_id, s.expires_at, u.id, u.email, u.display_name, u.role, u.status, u.verified,
  ...
  return async (req, res, next) => {
    ...
    if (token) {
      const row = await db.get(LOOKUP, sha256(token));
...
export async function startSession(db, config, res, userId) {
  ...
  const now = Date.now();
  (await db.run('INSERT INTO sessions (id, user_id, created_at, expires_at) VALUES (?, ?, ?, ?)', sha256(token),
    userId,
    now,
    now + SESSION_TTL_MS));
