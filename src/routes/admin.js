...
async function describeTarget(db, type, id) {
  if (type === 'user') {
    const u = (await db.get('SELECT id, display_name, status FROM users WHERE id = ?', id));
    return u ? { ownerId: u.id, preview: html`Member: <a href="/members/${u.id}">${u.display_name}</a> (${u.status})` } : null;
  }
  if (type === 'post') {
    const p = (await db.get('SELECT p.*, u.display_name FROM posts p JOIN users u ON u.id = p.author_id WHERE p.id = ?', id));
    return p
  ...
  if (type === 'comment') {
    const c = (await db.get('SELECT c.*, u.display_name FROM comments c JOIN users u ON u.id = c.author_id WHERE c.id = ?', id));
    return c
  ...
  if (type === 'message') {
    ...
    const m = (await db.get('SELECT m.*, u.display_name FROM messages m JOIN users u ON u.id = m.sender_id WHERE m.id = ?', id));
...
router.get('/admin', async (req, res) => {
  ...
  const stats = {
    members: (await count("SELECT COUNT(*) AS n FROM users WHERE status = 'active'")),
    newThisWeek: (await count('SELECT COUNT(*) AS n FROM users WHERE created_at > ?', weekAgo)),
    connections: (await count("SELECT COUNT(*) AS n FROM connections WHERE status = 'accepted'")),
    posts: (await count('SELECT COUNT(*) AS n FROM posts WHERE created_at > ?', weekAgo)),
    openReports: (await count("SELECT COUNT(*) AS n FROM reports WHERE status = 'open'")),
  };
  const byType = (await db.all("SELECT p.member_type, COUNT(*) AS n FROM profiles p JOIN users u ON u.id = p.user_id WHERE u.status = 'active' GROUP BY 1 ORDER BY 2 DESC"));
...
router.get('/admin/reports', async (req, res) => {
  ...
  const reports = (await db.all(`SELECT r.*, u.display_name AS reporter_name FROM reports r LEFT JOIN users u ON u.id = r.reporter_id
        WHERE r.status = ? ORDER BY r.created_at ${status === 'open' ? 'ASC' : 'DESC'} LIMIT 100`, status));
  const targets = await Promise.all(reports.map((r) => describeTarget(db, r.target_type, r.target_id)));
  res.page({
    title: 'Reports',
    wide: true,
...
async function suspend(db, actor, userId, why, demo = false) {
  ...
  (await db.run("UPDATE users SET status = 'suspended' WHERE id = ?", userId));
  (await endAllSessions(db, userId));
  (await audit(db, actor.id, 'user.suspend', `user:${userId} ${why}`));
  return true;
}

router.post('/admin/reports/:id/resolve', async (req, res, next) => {
  const { db } = req.app.locals;
  const id = v.id(req.params.id);
  const report = id && (await db.get("SELECT * FROM reports WHERE id = ? AND status = 'open'", id));
  ...
  const target = (await describeTarget(db, report.target_type, report.target_id));

  if (action === 'hide' && target?.hideable) {
    const table = report.target_type === 'post' ? 'posts' : 'comments';
    (await db.run(`UPDATE ${table} SET hidden = 1 WHERE id = ?`, report.target_id));
    (await audit(db, req.user.id, `${report.target_type}.hide`, `${report.target_type}:${report.target_id}`));
  }
  if (action === 'suspend' && target) {
    if (!(await suspend(db, req.user, target.ownerId, `report:${id}`, req.app.locals.config.demo))) {
      return res.status(403).type('text').send('You cannot suspend that account.');
    }
  }
  (await db.run('UPDATE reports SET status = ?, resolved_by = ?, resolution_note = ?, resolved_at = ? WHERE id = ?', action === 'dismiss' ? 'dismissed' : 'actioned',
    req.user.id,
    note || action,
    Date.now(),
    id));
  (await audit(db, req.user.id, `report.${action}`, `report:${id}`));
...
router.get('/admin/users', async (req, res) => {
  ...
  const users = (await db.all(`SELECT u.id, u.email, u.display_name, u.role, u.status, u.verified, u.created_at, p.member_type,
        ...
        ORDER BY u.created_at DESC LIMIT 100`, q, like, like));
  res.page({
    title: 'Members admin',
    wide: true,
...
router.post('/admin/users/:id', async (req, res, next) => {
  const { db } = req.app.locals;
  const id = v.id(req.params.id);
  const target = id && (await db.get('SELECT id, role FROM users WHERE id = ?', id));
  ...
  switch (action) {
    case 'verify':
    case 'unverify':
      (await db.run('UPDATE users SET verified = ? WHERE id = ?', action === 'verify' ? 1 : 0, id));
      (await audit(db, req.user.id, `user.${action}`, `user:${id}`));
      break;
    case 'suspend':
      if (!(await suspend(db, req.user, id, 'manual', req.app.locals.config.demo))) return deny();
      break;
    case 'reinstate':
      if (!isAdmin(req) && target.role !== 'member') return deny();
      (await db.run("UPDATE users SET status = 'active' WHERE id = ?", id));
      (await audit(db, req.user.id, 'user.reinstate', `user:${id}`));
      break;
    case 'role': {
      ...
      (await endAllSessions(db, id)); // privileges change → force a fresh login
      (await audit(db, req.user.id, 'user.role', `user:${id} ${role}`));
      break;
    }
    default:
...
async function hubsPage(req, errors = [], values = {}) {
  const { db } = req.app.locals;
  const hubs = (await db.all('SELECT h.*, (SELECT COUNT(*) FROM hub_members m WHERE m.hub_id = h.id) AS members FROM hubs h ORDER BY kind, name'));
  return {
    title: 'Hubs admin',
    wide: true,
...
router.get('/admin/hubs', async (req, res) => res.page((await hubsPage(req))));

router.post('/admin/hubs', async (req, res) => {
  const { db } = req.app.locals;
  ...
  else if ((await db.get('SELECT 1 FROM hubs WHERE slug = ?', slug))) errors.push('A hub with that name already exists.');
  if (errors.length) return res.status(400).page((await hubsPage(req, errors, { name, description })));
  (await db.run("INSERT INTO hubs (slug, name, kind, description, created_at) VALUES (?, ?, 'region', ?, ?)", slug, name, description, Date.now()));
  (await audit(db, req.user.id, 'hub.create', `hub:${slug}`));
...
router.get('/admin/audit', async (req, res, next) => {
  ...
  const { db } = req.app.locals;
  const rows = (await db.all('SELECT a.*, u.display_name FROM audit_log a LEFT JOIN users u ON u.id = a.actor_id ORDER BY a.id DESC LIMIT 200'));
  res.page({
    title: 'Audit log',
    wide: true,
