...
router.get('/members', requireAuth, (req, res) => {
  ...
  const interests = db.prepare('SELECT * FROM interests ORDER BY name').all();
  const f = {
  ...
  const rows = db
    .prepare(
      `SELECT u.id, u.display_name, u.verified, p.member_type, p.headline, p.city, p.country, p.open_to_intros
...
function loadMember(db, id) {
  return db
    .prepare(
      `SELECT u.id, u.display_name, u.verified, u.status, u.role, u.created_at, p.*
...
function profilePage(req, db, m, { errors = [], note = '' } = {}) {
  const uid = req.user.id;
  const self = m.id === uid;
  const conn = self ? null : connectionBetween(db, uid, m.id);
  const staff = req.user.role === 'admin' || req.user.role === 'moderator';
  const full = self || staff || m.visibility === 'members' || conn?.status === 'accepted';
  const interests = db
    .prepare('SELECT i.slug, i.name FROM interests i JOIN user_interests ui ON ui.interest_id = i.id WHERE ui.user_id = ? ORDER BY i.name')
    .all(m.id);
  const hubs = db
    .prepare("SELECT h.slug, h.name FROM hubs h JOIN hub_members hm ON hm.hub_id = h.id WHERE hm.user_id = ? AND h.kind = 'region'")
    .all(m.id);
  const iBlocked = !self && !!db.prepare('SELECT 1 FROM blocks WHERE blocker_id = ? AND blocked_id = ?').get(uid, m.id);
...
router.get('/members/:id', requireAuth, (req, res, next) => {
  const { db } = req.app.locals;
  const id = v.id(req.params.id);
  const m = id && loadMember(db, id);
  const staff = req.user.role === 'admin' || req.user.role === 'moderator';
  if (!m) return next();
  // Blocked (either direction) or suspended members are indistinguishable from
  // non-existent ones, except to moderators.
  if (!staff && m.id !== req.user.id && (m.status !== 'active' || isBlocked(db, req.user.id, m.id))) return next();
  res.page(profilePage(req, db, m));
});

router.post('/members/:id/connect', requireAuth, (req, res, next) => {
  const { db } = req.app.locals;
  const uid = req.user.id;
  const id = v.id(req.params.id);
  const m = id && loadMember(db, id);
  if (!m || m.id === uid || m.status !== 'active' || isBlocked(db, uid, m.id)) return next();
  ...
  if (connectionBetween(db, uid, m.id)) errors.push('There is already a connection or request between you.');
  ...
  const sentToday = db.prepare('SELECT COUNT(*) AS n FROM connections WHERE requester_id = ? AND created_at > ?').get(uid, since).n;
  ...
  if (errors.length) return res.status(400).page(profilePage(req, db, m, { errors, note }));

  db.prepare('INSERT INTO connections (requester_id, addressee_id, note, created_at) VALUES (?, ?, ?, ?)').run(uid, m.id, note, Date.now());
  audit(db, uid, 'connection.request', `user:${m.id}`);
  res.flash('request-sent');
  res.redirect(303, `/members/${m.id}`);
});

router.post('/members/:id/block', requireAuth, (req, res, next) => {
  const { db } = req.app.locals;
  const uid = req.user.id;
  const id = v.id(req.params.id);
  if (!id || id === uid || !db.prepare('SELECT 1 FROM users WHERE id = ?').get(id)) return next();
  db.exec('BEGIN');
  try {
    db.prepare('INSERT OR IGNORE INTO blocks (blocker_id, blocked_id, created_at) VALUES (?, ?, ?)').run(uid, id, Date.now());
  ...
  audit(db, uid, 'user.block', `user:${id}`);
  res.flash('blocked');
  res.redirect(303, '/members');
});

router.post('/members/:id/unblock', requireAuth, (req, res, next) => {
  const { db } = req.app.locals;
  const id = v.id(req.params.id);
  if (!id) return next();
  db.prepare('DELETE FROM blocks WHERE blocker_id = ? AND blocked_id = ?').run(req.user.id, id);
  res.flash('unblocked');
